...
- **Diseño:** sistema de clases compartido (`app-shell`, `surface-panel`, `btn-marr`, `page-hero`, `admin-page`) con soporte **modo claro/oscuro**; navbar **sticky** (sin `fixed` + padding compensatorio).
- **Layout:** páginas admin comparten patrón: franja hero (`page-hero`), contenido en **`max-w-6xl mx-auto`**, tarjetas con `surface-panel`, overlay “Updating…” al recargar listas paginadas, paginación con `TablePagination`.
- **Informe de ventas / gráfico:** moneda y tooltips con formato **`en-US`**; eje Y del gráfico con prefijo **`$`** legible; API de resumen mensual con etiquetas de mes en **`en-US`** (`MMM yyyy`); gráficos adicionales por categoría y material; tabla de top productos.
...
2) **Detalle de producto real** (consumo API, galería, CTA) — *hecho (galería multi-imagen, junio 2026)*  
3) **Mis pedidos** (cliente) + Admin pedidos (estados) — *hecho*  
4) **Materiales (tabla) + UI de administración** — *hecho*  
4b) **Contacto persistido + admin** — *hecho (junio 2026)*  
4c) **Fase B (junio 2026):** galería multi-imagen, checkout con dirección/impuestos, reportes por categoría/material y top productos, **diseño unificado** claro/oscuro — *hecho*  
...
*Última revisión: junio 2026. Código y **UI del panel admin** en inglés; API en inglés. Este plan y las guías operativas en español.*

### 12.1 Ya implementado (resumen)
| Área | Qué hay |
|------|---------|
| Auth | Registro, login, JWT, **AuthContext** en frontend, interceptor 401, roles **Admin / Employee / Customer**; cuentas inactivas no hacen login; gestión de usuarios **solo Admin**. |
| Catálogo API | CRUD productos (multipart + Cloudinary), **DTOs**, listado paginado, filtros categoría/material/precio/stock, `GET` por id, **`imageUrls` / `images[]`**, galería en `ProductImages`. |
| Materiales | Tabla **Materials**, FK **MaterialId**, `GET /api/materials`. |
| Categorías | Tabla **Categories**, `GET /api/categories`. |
| Pedidos | Crear (catálogo + personalizado), mis pedidos, admin listado/detalle/estado/delete; **transacciones** y **StockVersion** (concurrencia); **subtotal, impuestos, envío y dirección** en pedidos de catálogo. |
| Carrito / checkout | localStorage, sync de stock al checkout y al volver a la pestaña, toast, `/cart`, **formulario de envío**, cotización `POST /api/checkout/quote`. |
| Cuenta | Perfil, cambio de contraseña; rutas `/profile`, `/orders` protegidas. |
| Admin | Productos (soft-delete, **galería multi-imagen**), pedidos, dashboard KPIs + gráfico, **informe ventas** (mensual + por categoría/material + top productos), usuarios, **mensajes de contacto** en Settings. |
| UX / diseño | Sistema de clases unificado (`app-shell`, `surface-panel`, `btn-marr`, etc.), modo claro/oscuro, navbar sticky, menú usuario sin bloqueo de scroll. |
| Seguridad UX | `/dashboard` y `/admin/*` protegidos por rol; página 404; rate limit en auth y contacto. |
| Infra | Dockerfile backend y frontend, `docker-compose.yml`, CI GitHub Actions, tests integración (7+). |
| Documentación | `PERMISOS-Y-API.md`, `DESPLIEGUE.md`, `JoyeriaBackend.http`, README. |

### 12.2 Pendiente (MVP producción → producto completo)
| Prioridad | Backend | Frontend |
|-----------|---------|----------|
| **Hecho Fase A** | Contacto persistido (`ContactMessages`), admin listado | Formulario `/contact` + Settings |
| **Hecho Fase B** | `ProductImages`, checkout (dirección, impuestos, envío), reportes categoría/material/top | Galería ficha + admin, carrito ampliado, gráficos Sales |
| Media | Proveedores, coste, margen | Admin proveedores |
| Media | Pagos (Stripe/PayPal) | UI de pago |
| Baja | Facturas, devoluciones, certificados, CRM citas/reparaciones | Pantallas asociadas |
| Transversal | i18n / multi-moneda (§1.3) | — |
| Transversal | Más tests E2E; `npm audit` periódico | Playwright/Cypress (opcional) |

### 12.3 Deuda técnica conocida
- **Impuestos:** tasas fijas por país en `appsettings`; sin reglas por estado/provincia ni exenciones.
- **Material.AlloyOrFineness**: en modelo; UI solo usa nombre.
- **CRA** (no Vite); Tailwind/Headless en `devDependencies` del frontend.
